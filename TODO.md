...
### Item 40: Set `RESEND_AUDIENCE_ID` on Netlify

- **Status:** ⬜ Not started (added 2026-06-23)
- **Why:** During the Netlify environment-variable setup on 2026-06-23, `RESEND_API_KEY` was populated but `RESEND_AUDIENCE_ID` imported empty. Without the audience ID, newsletter signups (Netlify Forms → `submission-created` function → Resend) will not be enrolled into the Resend audience. The single-opt-in welcome email may still send, but the contact is never added to the audience list, so the subscriber base silently fails to grow.
- **Action:**
  1. In the Resend dashboard, open Audiences → the GovCompass audience → copy its ID.
  2. Set it on the Netlify site, either by adding it to `.env` and re-running `netlify env:import .env`, or directly: `netlify env:set RESEND_AUDIENCE_ID <id>`.
  3. Verify with `netlify env:get RESEND_AUDIENCE_ID` and trigger a redeploy for it to take effect.
- **Notes:** All other Clerk/Resend/GA4 env vars are set and verified. Clerk keys are split per context (production = live, non-prod = test). See `docs/CICD_PIPELINE.md` and the project memory for the full Netlify setup.
