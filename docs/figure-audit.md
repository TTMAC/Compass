...
- ~~`1-3`: "over 60 municipalities under Section 139 (1998–2023)"~~ → **✅ RESOLVED 2026-06-29: there is no authoritative consolidated cumulative count, so replaced with the Parliamentary Budget Office's verifiable figure — 39 municipalities under Section 139 between 2020/21 and 2022/23 (some dating to 2015/16; NW/FS/EC most affected).** Source: PBO, _Section 139 and 154 Municipal Interventions_, May 2025. **⚠️ Newly found:** `3-3-municipal-councils` separately states "more than 70 municipalities since democratic local government (COGTA)" — a cumulative figure I could not independently verify (plausible; different measure from the PBO recent window). Left as-is, flagged.
...
- **Citation pass — ✅ APPLIED 2026-06-29 (commit `001f674`, Phase 2 batch 3):** `eg-1-1` 28M grants→SASSA, median income→Stats SA; `eg-1-2` 1970s ~10× per-pupil gap→apartheid-era budgets; `eg-1-3` Botswana 12km/<100-graduates→AJR "An African Success Story: Botswana" (Resources); `hd-5-2` "20,000 doctors abroad" re-attributed HPCSA→OECD/SAMP. `hd-5-1` comparators (incl. Korea 1953 $67) already sourced via its Resources — no edit needed.
- **Owner-source-resolved items — ✅ ALL APPLIED:** `ss-2-4` femicide (SAMRC/UN Women, `da9b8f1`); `2-3` Eskom municipal debt → Treasury R56.3bn Dec-2022 (`11e8bbe`); `eg-4-2` manufacturing employment → Stats SA formal survey (`2244783`); `1-3` Section 139 → PBO May-2025 figure (`21c8fba`); `2-3` consumer debt → Treasury Section 71 R416.1bn Mar-2025 (`37ce157`).
- **EG 🕒 confirm-then-edit list — ✅ APPLIED (web-verified):** `eg-2-1` citrus → SA #1 in 2025 (CGA); `eg-2-4` STEM grads Korea 600k→~140k, Poland 200k→~65k, per-100k Korea ">1,000"→">250"; `eg-2-4` German apprentices 300k→~480k; `eg-2-5` R&D Korea 4.8%→5.1%, Israel 5.4%→6.8% (OECD MSTI 2024), fixed "Korea highest in world"→"second to Israel".
- **TIMSS/PIRLS + S139 attribution — ✅ APPLIED:** `ra-2-1`/`3-2` TIMSS 2019→2023 (Gr5 maths 374→362); `3-2` PIRLS 2016/78%→2021/81%; `hd-3-1` Gr9 maths ~370→~390; `3-3` S139 dropped wrong "COGTA" attribution → PARI (>130 since 1998).
- **Remaining (optional cosmetic, deferred):** the broad year-label polish — adding "(2024/25)" tags to budget/grant figures that are already verified correct, and standardising every generic PIRLS/TIMSS mention. Low value, not errors; can be done in a future maintenance pass. **No known factual errors remain open.**
